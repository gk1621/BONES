...
- **Camera vs. keyboard:** the game's *primary* control is webcam body/hand tracking (MediaPipe pose + hand landmarkers). Keyboard mode is the intentional fallback. A headless cloud VM has no webcam, so use Keyboard Demo Mode for functional gameplay testing. To exercise the real camera + MediaPipe pipeline headlessly, launch Chrome with `--use-fake-device-for-media-stream --use-fake-ui-for-media-stream` and click `Start`; the pipeline initializes and reports `frame.source: "camera"` via `Export Diagnostics`, but the synthetic feed contains no human so it detects `0` poses (0% confidence) — real body-movement gameplay requires an actual camera + person.
